...
module.exports = async (interaction) => {
    ...
    if (customId === 'setup_menu_permissions') {
        ...
        const embed = new EmbedBuilder().setTitle('🔐 Permissions Configuration').setDescription('Choose a sub-category to configure.').setColor('#9B59B6');
    ...
    if (customId === 'setup_menu_protection') {
        ...
        const embed = new EmbedBuilder().setTitle('🛡️ Protection Systems').setDescription('Configure server defense and emergency systems.').setColor('#E74C3C');
        const row = new ActionRowBuilder().addComponents(
    ...
    if (customId === 'setup_lockdown_menu') {
        ...
        const embed = new EmbedBuilder().setTitle('🔒 Lockdown Configuration').setDescription(`Select channels to lock/hide during \`/lockdown\`.\n\n**Current:**\n${currentChannels}`).setColor('#E74C3C');
