...
module.exports = async (interaction) => {
    ...
    if (customId === 'setup_staff' || customId === 'setup_staff_menu') {
        ...
        const currentRoles = res.rows[0]?.staff_roles ? res.rows[0].staff_roles.split(',').map(r => `<@&${r}>`).join(', ') : '`None`';
        const commandList = STAFF_COMMANDS.map(c => `\`${c}\``).join(', ');

        const embed = new EmbedBuilder().setTitle('🛡️ Staff Roles').setDescription(`Roles with Staff access.\n\n**Current:**\n${currentRoles}`).addFields({ name: 'Commands', value: commandList }).setColor('#3498DB');
        const menu = new RoleSelectMenuBuilder().setCustomId('select_staff_roles').setPlaceholder('Select roles...').setMinValues(0).setMaxValues(25);
        const back = new ButtonBuilder().setCustomId('setup_menu_permissions').setLabel('Back').setStyle(ButtonStyle.Secondary);
        await interaction.editReply({ embeds: [embed], components: [new ActionRowBuilder().addComponents(menu), new ActionRowBuilder().addComponents(back)] });
        return;
    }
